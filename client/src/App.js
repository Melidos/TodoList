...
export default class App extends Component {
  ...
  componentDidMount() {
    ...
    axios
      ...
      .then((user) => this.setState({ userLogged: user.data }))
      .then((_) => {
  ...
  exportToCSV = () => {
    const csvData =
      ",completed,name\n" +
      this.state.todos
        .map((el) => el.completed + ',"' + el.name.replace(/"/gi, "'") + '"')
        .join("\n");
  ...
  markComplete = (id) => {
    axios
      ...
      .catch((err) => console.error("Couldn't update state: " + err));
  };
  ...
  addTodo = (completed, name) => {
    axios
      ...
      .then((todo) =>
        this.setState({
          todos: [todo.data, ...this.state.todos],
        })
      )
      .catch((err) => console.error("Couldn't create new todo: " + err));
  };
    ...
    return (
      ...
      >
        <Container>
          ...
          >
            Use the menu to login to use the app
          </h1>
        </Container>
      </ThemeProvider>
    );
